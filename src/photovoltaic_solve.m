...
%lb=zeros(7*T,1);
lb=[zeros(T+T-1,1); stock_initial ; zeros(5*T,1)];
ub=[I*Inf;
    ones(T-1,1)*Inf;
    stock_initial;
    I*nb_max_heure_sup;
...
A=[E*duree_assemblage/60 zeros(T) E*-nb_ouvriers*5 zeros(T,4*T)];
b=ones(T,1)*35*nb_ouvriers;

Aeq=[E -E zeros(T) E -E -E -E;
     zeros(T,4*T) E zeros(T,1) E(:,1:end-1) zeros(T,2) E(:,1:end-2)];
beq=[-stock_initial;
    zeros(T-1,1);
     demande'];
